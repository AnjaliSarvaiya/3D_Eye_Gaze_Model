...
# # -*- coding: utf-8 -*-
# """
# Train & Validate entrypoint that reads args/paths from config.py.

# - Uses PKL split (train/valid/test) if args['use_pkl_for_dataload'] is True.
# - Supports resume training via args['continue_training'] or args['weights_path'].
# - W&B is hard-disabled (scripts.py may call wandb.log; we stub it to no-op).
# - Keeps the rest of your training/validation logic identical to main.py.
# """

# # --- Disable/Stub W&B before any other imports that may touch it -------------
# import os as _os
...
# # keep your import path as in main.py
...
# # read args/paths from your config.py
...
# def _ensure_dirs(paths_dict):
#     for k in ["exp", "results", "logs", "figures"]:
#         if k in paths_dict and paths_dict[k]:
#             os.makedirs(paths_dict[k], exist_ok=True)
...
#     # deactivate tensorboard images in multi-rank
#     # (you can flip this back to rank_cond if you want TB logs)
#     rank_cond = False

#     net_dict = []

#     # %% Build model (identical logic)
#     if args['use_frn_tlu']:
...
#         net = model_dict[args['model']](args, norm='group_norm', act_func=F.leaky_relu)
...
#     # %% Load initial weights (resume/pretrained)
...
#         net_dict = torch.load(path_pretrained, map_location='cpu')
#         state_dict_single = move_to_single(net_dict['state_dict'])
#         net.load_state_dict(state_dict_single, strict=False)
#         print(f'[init] loaded weights from: {path_pretrained}')

#     # %% Mode-specific weight load
#     if test_mode or validation_mode:
#         if args['path_model']:
#             net_dict = torch.load(args['path_model'], map_location='cpu')
#         else:
#             net_dict = torch.load(os.path.join(path_dict['results'], 'last.pt'), map_location='cpu')
#         state_dict_single = move_to_single(net_dict['state_dict'])
#         net.load_state_dict(state_dict_single, strict=False)
#         writer = []
#     else:
...
#     if not test_mode and not validation_mode:
#         logger.write('Training!')
#         # was: wandb.watch(net) — now a no-op
#         wandb.watch(net)
...
#     # -------- Data (PKL path or MasterKey) -----------------------------------
#     if args['use_pkl_for_dataload']:
#         # PKL override from config.get_paths() if present
#         pkl_path = None
#         if 'pkl_override' in path_dict and path_dict['pkl_override']:
#             pkl_path = path_dict['pkl_override']
#         else:
#             pkl_path = os.path.join(path_dict['repo_root'], 'cur_objs', args['mode'], f"cond_{args['cur_obj']}.pkl")
#         assert os.path.isfile(pkl_path), f"[DATA] Could not find split PKL at: {pkl_path}"

#         with open(pkl_path, 'rb') as f:
#             train_obj, valid_obj, test_obj = pickle.load(f)
#     else:
#         # Old MasterKey flow (unchanged)
...
#     # Optional overlap handling (kept as in your main.py)
#     print('Starting the procedure of removing unwanted train-val video overlap...')
#     train_vid_ids = list(np.unique(train_obj.imList[:, :, 1]))
#     # (overlap filter was commented out in your code)

#     # Limit test set for speed (kept)
...
#     # Samplers
#     train_sampler = DistributedSampler(train_obj, rank=args['local_rank'],
#                                        shuffle=False, num_replicas=args['world_size'])
#     valid_sampler = DistributedSampler(valid_obj, rank=args['local_rank'],
#                                        shuffle=False, num_replicas=args['world_size'])
#     test_sampler  = DistributedSampler(test_obj,  rank=args['local_rank'],
#                                        shuffle=False, num_replicas=args['world_size'])
...
#     elif test_mode:
#         test_loader = DataLoader(test_obj, shuffle=False, num_workers=0,
#                                  drop_last=True, batch_size=args['batch_size'],
#                                  sampler=test_sampler if args['do_distributed'] else None)
#     else:
#         train_loader = DataLoader(train_obj, shuffle=args['random_dataloader'],
...
#     if args['adv_DG']:
#         param_list_adv = [p for n, p in net.named_parameters() if 'adv' in n]
#         optimizer_disc = torch.optim.Adam(param_list_adv, lr=args['lr'], amsgrad=True)
#     else:
...
#         outp = args['save_results_here'] or os.path.join(path_dict['results'], 'test_results.pkl')
...
#                                last_epoch_valid=True, csv_save_dir=path_dict['exp'])
#         checkpoint['valid_result'] = valid_result

#         os.makedirs(path_dict['results'], exist_ok=True)
#         outp = args['save_results_here'] or os.path.join(path_dict['results'], 'valid_results.pkl')
#         with open(outp, 'wb') as f:
#             pickle.dump(checkpoint, f)
#         return
...
#     # resume training (optimizer + epoch)
...
#                                batches_per_ep=valid_batches_per_ep, last_epoch_valid=last_epoch_validation)
...
#         # save "last.pt" every save_every epochs
...
#             early_stop.save_checkpoint(temp_score, checkpoint, update_val_score=False,
#                                        use_this_name_instead='last.pt')

#         if use_sched:
#             # some schedulers want epoch as kw
#             try:
...
# def main():
#     # pull args/paths from config.py
#     args  = cfg.get_args()
#     paths = cfg.get_paths()
...
#     # replicate a couple of run-time tweaks from run.py
#     if args.get('use_pkl_for_dataload', False):
#         # pkl splits are typically made for fixed frames
#         args['frames'] = args.get('frames', 4)

#     # DDP
#     if args.get('do_distributed', 0):
...
#     args['batch_size'] = int(args['batch_size'] / world_size) if world_size > 0 else args['batch_size']
...
#     # attach repo_root/path_data from paths (kept same names)
#     paths.setdefault('repo_root', os.path.abspath(os.path.dirname(__file__)))
#     paths.setdefault('path_data', args['path_data'])

#     # Training / Validation / Test flow (same as run.py intent)
#     if not args.get('only_test', 0):
#         if not args.get('only_valid', 0):
#             print('train mode')
#             train(args, paths, validation_mode=False, test_mode=False)

#             print('validation mode')
#             train(args, paths, validation_mode=True, test_mode=False)

#             print('test mode')
#             train(args, paths, validation_mode=False, test_mode=True)

#     # If user ran with only_valid or only_test toggles
#     if args.get('only_valid', 0):
#         print('validation mode')
#         train(args, paths, validation_mode=True, test_mode=False)
#     if args.get('only_test', 0):
#         print('validation mode')
#         train(args, paths, validation_mode=True, test_mode=False)
#         print('test mode')
#         train(args, paths, validation_mode=False, test_mode=True)

#     # wandb.finish() is a no-op due to our stub
#     wandb.finish()
...
"""
train.py
- Loads args/paths from config.py
- Creates a timestamped experiment folder under Results/
- Runs Train -> Valid -> Test
- Disables wandb interactions (no login/init needed)
"""
