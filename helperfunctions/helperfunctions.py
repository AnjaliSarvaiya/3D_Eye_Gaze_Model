...
def extract_datasets(subsets):
    ...
    # nditer sometimes complains on empty; keep flags explicit
    ds_idx = [str(ele).split('_')[0] for ele in np.nditer(subsets, flags=['refs_ok'], op_flags=['readonly'])]
    ds_present, ds_index = np.unique(ds_idx, return_inverse=True)
    return ds_present, ds_index
...
# def convert_to_list_entries(data_dict):
#     """
#     Converts a dictionary of numpy arrays (or scalars) into
#     a list of per-sample dictionaries. Scalars are repeated
#     for all samples, and arrays of mismatched lengths are skipped.
#     """

#     # Convert torch tensors to numpy
#     for key, item in data_dict.items():
#         if 'torch' in str(type(item)):
#             data_dict[key] = item.detach().cpu().numpy()

#     # Find the majority length (most common array size)
#     lengths = [v.shape[0] for v in data_dict.values() if isinstance(v, np.ndarray)]
#     if not lengths:  # No arrays, just return a single dict
#         return [data_dict]

#     from collections import Counter
#     max_len = Counter(lengths).most_common(1)[0][0]
...
#                     entry[key] = item[ii]
#                 elif item.shape[0] == 1:  # repeat singletons
#                     entry[key] = item[0]
...
def convert_to_list_entries(data_dict):
    ...
    # Determine the minimum valid length (avoid IndexError)
    valid_keys = [key for key, val in data_dict.items()
                  if isinstance(val, np.ndarray) and val.ndim > 0]
    if not valid_keys:
        return []

    num_entries = min([data_dict[key].shape[0] for key in valid_keys])

    # Create list of dicts for each entry
    out = []
    for ii in range(num_entries):
        entry = {key: data_dict[key][ii]
                 for key in valid_keys
                 if ii < data_dict[key].shape[0]}
        out.append(entry)
    return out
