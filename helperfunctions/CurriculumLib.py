...
class DataLoader_riteyes(Dataset):
    def __init__(self,
        ...
        # --- GROUP BY ARCHIVE BEFORE FORMING CLIPS ---
# self.imList currently has shape (N, 3) with columns [im_num, arch_idx, ds_idx]
        if self.init_frames > 1:
            clips = []
            for arch_id in np.unique(self.imList[:, 1]):
                rows = self.imList[self.imList[:, 1] == arch_id]
                if rows.shape[0] < self.init_frames:
                    continue
                rem = rows.shape[0] % self.init_frames
                if rem:
                    rows = rows[:-rem]
                if rows.size == 0:
                    continue
                rows = rows.reshape(-1, self.init_frames, 3)
                clips.append(rows)
            if len(clips) == 0:
                self.imList = np.empty((0, self.init_frames, 3), dtype=self.imList.dtype)
            else:
                self.imList = np.concatenate(clips, axis=0)
        else:
            # frames==1: keep as (N,1,3) for consistency
            self.imList = self.imList.reshape(-1, 1, 3)
    ...
    def sort(self, sort, batch_size=None):
        ...
        if sort=='ordered':
            ...
            temp_imList = np.stack(temp_imList, axis=1).reshape(-1, self.init_frames, 3)
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            data_dict['image_raw_u8'] = data_dict['image'].copy().astype(np.uint8)
        ...
        if (np.all(data_dict['im_num']>=0)):
            pic = data_dict['image'].astype(np.float32)
            mean = pic.mean(dtype=np.float64)
            std  = pic.std(dtype=np.float64)

            if std < 1e-6:
    # flat/constant image; mark bad and zero out
                data_to_torch = np.zeros_like(pic, dtype=np.float32)
                data_dict['is_bad'] = np.ones((self.init_frames,), dtype=bool)
            else:
                data_to_torch = (pic - mean) / (std + 1e-6)
                if not np.isfinite(data_to_torch).all():
                    data_to_torch = np.zeros_like(pic, dtype=np.float32)
                    data_dict['is_bad'] = np.ones((self.init_frames,), dtype=bool)
...
#read entry works. Create the 3D volume for input to the nn
    def readEntry_new(self, idx):
        im_num  = self.imList[idx, ..., 0]        # (F,)
        set_num = self.imList[idx, ..., 2]        # (F,)
        arch_num = self.imList[idx, ..., 1]       # (F,)

        archStr = self.arch[arch_num[0]]
        archName = archStr.split(':')[0]

    # cache H5 handle
        if archName not in self.fileObjs:
            self.fileObjs[archName] = h5py.File(
                os.path.join(self.path2data, f"{archName}.h5"), "r", swmr=True
            )
        f = self.fileObjs[archName]

        F = self.init_frames
    # ---- Images (F,H,W)
        image = f["Images"][im_num, ...].astype(np.float32)
        H, W = image.shape[1], image.shape[2]

    # ---- Pupil center (F,2)
        if "pupil_loc" in f and f["pupil_loc"].shape[0] > 0:
            pupil_center = f["pupil_loc"][im_num, ...].astype(np.float32)
            pupil_center_available = [True] * F
        else:
            pupil_center = -np.ones((F, 2), dtype=np.float32)
            pupil_center_available = [False] * F

    # ---- Mask without skin (F,H,W)
        if self.train_with_mask:
            if "Masks_noSkin" in f and f["Masks_noSkin"].shape[0] > 0:
                mask_noSkin = f["Masks_noSkin"][im_num, ...].astype(np.int16)
            # must contain both iris(2) and pupil(3)
                any_pupil = np.any(mask_noSkin == 3)
                any_iris  = np.any(mask_noSkin == 2)
                if not (any_pupil and any_iris):
                    mask_noSkin = -np.ones((F, H, W), dtype=np.int16)
                    mask_available = [False] * F
                else:
                    mask_available = [True] * F
            else:
                mask_noSkin = -np.ones((F, H, W), dtype=np.int16)
                mask_available = [False] * F

    # ---- Ellipse fits (F,5)
        if "Fits" in f and "pupil" in f["Fits"] and f["Fits"]["pupil"].shape[0] > 0:
            pupil_param = f["Fits"]["pupil"][im_num, ...].astype(np.float32)
            pupil_ellipse_available = [True] * F
        else:
            pupil_param = -np.ones((F, 5), dtype=np.float32)
            pupil_ellipse_available = [False] * F

        if "Fits" in f and "iris" in f["Fits"] and f["Fits"]["iris"].shape[0] > 0:
            iris_param = f["Fits"]["iris"][im_num, ...].astype(np.float32)
            iris_ellipse_available = [True] * F
        else:
            iris_param = -np.ones((F, 5), dtype=np.float32)
            iris_ellipse_available = [False] * F

    # ---- Optional features (Dikablis only)
        more_features = ("Dikablis" in archName)

        if more_features and "Eyeball" in f and f["Eyeball"].shape[0] > 0:
            eyeball = f["Eyeball"][im_num, ...].astype(np.float32)       # (F,4)
            eyeball_available = [True] * F
        else:
            eyeball = -np.ones((F, 4), dtype=np.float32)
            eyeball_available = [False] * F

        if more_features and "Gaze_vector" in f and f["Gaze_vector"].shape[0] > 0:
            gaze_vector = f["Gaze_vector"][im_num, ...].astype(np.float32)  # (F,3)
            gaze_vector_available = [True] * F
        else:
            gaze_vector = -np.ones((F, 3), dtype=np.float32)
            gaze_vector_available = [False] * F

        if more_features and "pupil_lm_2D" in f and f["pupil_lm_2D"].shape[0] > 0:
            pupil_lm_2D = f["pupil_lm_2D"][im_num, ...].astype(np.float32)  # (F,17)
            pupil_lm_2D_available = [True] * F
        else:
            pupil_lm_2D = -np.ones((F, 17), dtype=np.float32)
            pupil_lm_2D_available = [False] * F

        if more_features and "pupil_lm_3D" in f and f["pupil_lm_3D"].shape[0] > 0:
            pupil_lm_3D = f["pupil_lm_3D"][im_num, ...].astype(np.float32)  # (F,25)
            pupil_lm_3D_available = [True] * F
        else:
            pupil_lm_3D = -np.ones((F, 25), dtype=np.float32)
            pupil_lm_3D_available = [False] * F

        if more_features and "iris_lm_2D" in f and f["iris_lm_2D"].shape[0] > 0:
            iris_lm_2D = f["iris_lm_2D"][im_num, ...].astype(np.float32)    # (F,17)
            iris_lm_2D_available = [True] * F
        else:
            iris_lm_2D = -np.ones((F, 17), dtype=np.float32)
            iris_lm_2D_available = [False] * F

        if more_features and "iris_lm_3D" in f and f["iris_lm_3D"].shape[0] > 0:
            iris_lm_3D = f["iris_lm_3D"][im_num, ...].astype(np.float32)    # (F,25)
            iris_lm_3D_available = [True] * F
        else:
            iris_lm_3D = -np.ones((F, 25), dtype=np.float32)
            iris_lm_3D_available = [False] * F
    ...
    # ---- Pack
        ...
        data_dict["gaze_vector"] = gaze_vector
        data_dict["eyeball"] = eyeball
        data_dict["pupil_lm_2D"] = pupil_lm_2D
        data_dict["pupil_lm_3D"] = pupil_lm_3D
        data_dict["iris_lm_2D"] = iris_lm_2D
        data_dict["iris_lm_3D"] = iris_lm_3D

        is_bad_list = [False] * F
        if self.train_with_mask:
        # if mask has invalid label range AND claimed available → mark bad
            if (np.any(data_dict["mask"] < 0) or np.any(data_dict["mask"] > 3)) and np.all(mask_available):
                is_bad_list = [True] * F
        ...
        if self.train_with_mask:
            data_dict["mask_available"] = np.stack(mask_available, axis=0)
        data_dict["pupil_center_available"] = np.stack(pupil_center_available, axis=0) \
            if not np.all(pupil_center == -1) else np.zeros((F,), dtype=bool)
        data_dict["iris_ellipse_available"] = np.stack(iris_ellipse_available, axis=0) \
            if not np.all(iris_param == -1) else np.zeros((F,), dtype=bool)
        data_dict["pupil_ellipse_available"] = np.stack(pupil_ellipse_available, axis=0) \
            if not np.all(pupil_param == -1) else np.zeros((F,), dtype=bool)

        data_dict["is_bad"] = np.array(is_bad_list, dtype=bool)
...
#     AllDS = []
#     for chunk in D:
#         if not chunk.endswith(".pkl"):
#             continue
#         with open(os.path.join(path2arc_keys, chunk), 'rb') as f:
#             data = pickle.load(f)
#             if isinstance(data, list):
#                 AllDS.extend(data)  # Each .pkl contains a list of dicts
#             elif isinstance(data, dict):
#                 AllDS.append(data)  # Fallback if a single dict
#     return AllDS
...
#     # if archNum.size == 0:
#     #     print("⚠ Warning: No valid bins found during pupil location stratification. Skipping stratification.")
#     #     return np.arange(len(AllDS)), AllDS
...
def generate_strat_indices(AllDS):
    """
    Prune obvious outliers near image borders (very lightly), then stratify
    over a 2D center grid and archive id. Robust against empty/edge cases.
    """
    import numpy as np
    import copy

    # ---- 1) Choose centers: pupil if present else iris
    pupil_missing = np.any(AllDS['pupil_loc'] < 0, axis=1)
    centers = AllDS['pupil_loc'].copy()
    centers[pupil_missing, :] = AllDS['iris_loc'][pupil_missing, :]

    n0 = centers.shape[0]
    if n0 == 0:
        print('⚠ No samples at stratification start; skipping.')
        return np.arange(0), AllDS

    # ---- 2) Gentle border pruning (1%), with guard so we never delete "too much"
    border = 0.01  # was 0.10; 10% was nuking your TEyeD subsets
    near_border = np.any((centers < border) | (centers > 1 - border), axis=1)
    keep_mask = ~near_border

    # If this would drop more than half the data, SKIP the filter entirely
    if keep_mask.sum() < 0.5 * len(keep_mask):
        print(f"⚠ Border filter would drop {near_border.sum()} / {len(keep_mask)} samples — skipping border pruning.")
        keep_mask[:] = True

    if not np.all(keep_mask):
        AllDS = rmEntries(AllDS, ~keep_mask)
        centers = centers[keep_mask]

    n = centers.shape[0]
    if n == 0:
        print('⚠ No samples after pruning; skipping stratification.')
        return np.arange(0), AllDS

    # ---- 3) Archive inverse labels (same length as current AllDS)
    _, _, archNum = np.unique(AllDS['archive'], return_index=True, return_inverse=True)
    if archNum.size == 0:
        print('⚠ No valid "arch" labels; skipping stratification.')
        return np.arange(n), AllDS

    # ---- 4) Stable binning (avoid digitize edge-cases)
    numBins = 5
    binx = np.clip((centers[:, 0] * numBins).astype(int), 0, numBins - 1)
    biny = np.clip((centers[:, 1] * numBins).astype(int), 0, numBins - 1)
    K_arch = int(archNum.max()) + 1

    indx = np.ravel_multi_index((binx, biny, archNum),
                                (numBins, numBins, K_arch))

    print('Original # of entries:', n)

    # ---- 5) Optional: drop ultra-rare cells (<=2) BUT don’t empty the set
    uniq, counts = np.unique(indx, return_counts=True)
    small = uniq[counts <= 2]
    if small.size > 0 and (np.isin(indx, small).sum() < n - 10):  # keep at least 10
        keep2 = ~np.isin(indx, small)
        indx = indx[keep2]
        AllDS = rmEntries(AllDS, ~keep2)

    print('# of entries after stratification:', indx.size)
    return indx, AllDS
...
def generate_fileList(AllDS, mode='vanilla', notest=True):
    try:
        ...
        print(f"⚠ Stratification failed ({e}); falling back to sequential indexing.")
        indx = np.arange(AllDS['im_num'].shape[0], dtype=int)

# If stratification produced nothing, fall back to "no pruning"
    if indx.size == 0:
        print("⚠ Stratification produced 0 samples; falling back to sequential indexing (no pruning).")
        indx = np.arange(AllDS['im_num'].shape[0], dtype=int)
