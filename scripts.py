...
def forward(net,
    ...
    for bt in range(batches_per_ep):
        # print(f"[DEBUG] Processing batch {bt}")

        start_time = time.time()

        try:
            data_dict = next(loader_iter)
            # print(f"[DEBUG] Loaded batch successfully. Image shape: {data_dict['image'].shape}")
        except:
            print('Loader reset')
            loader_iter = iter(loader)
            data_dict = next(loader_iter)
        ...
        if torch.any(data_dict['is_bad']):
            print(f"[SKIPPED] Batch {bt}: Bad batch found. Replacing with a good sample.")
        ...
        if args['cur_obj'] !='Ours':
            try:
                ...
                print(f"[SKIPPED] Batch {bt}: Invalid pupil_ellipse orientation.")
                continue
            assert torch.all(data_dict['pupil_ellipse'][:, :, -1] >= 0), \
        ...
        with torch.autograd.set_detect_anomaly(bool(args['detect_anomaly'])):
            with torch.cuda.amp.autocast(enabled=bool(args['mixed_precision'])):
                ...
                if mode == 'train':
                    ...
                    # print(f"[DEBUG] out_dict_valid: {out_dict_valid}")
                else:
                    with torch.no_grad():
                        out_dict, out_dict_valid = net(data_dict, args)
                        # print(f"[DEBUG] out_dict_valid: {out_dict_valid}")
                        # print(f"[DEBUG] train_with_mask: {train_with_mask}, net_rend_head: {args['net_rend_head']}, net_ellseg_head: {args['net_ellseg_head']}")

                
                if not out_dict_valid:
                    print(f"[SKIPPED] Batch {bt}: out_dict_valid=False")
                    continue

                if torch.all(data_dict['image']==0):
                    print(f"[SKIPPED] Batch {bt}: Image is all zeros")
                    continue
                ...
                if args['net_rend_head']:
                    if torch.isnan(out_dict['T']).any() or torch.isinf(out_dict['T']).any():
                        print(f"[SKIPPED] Batch {bt}: NaN or Inf in T.")
                        continue

                        print(f"[OK] Batch {bt} passed all checks, collecting metrics now.")
        ...
        if (mode == 'test') and args['save_test_maps']:
            ...
            # only keep per‐sample arrays (drop scalar floats like loss)
            raw = merge_two_dicts(detach_cpu_numpy(out_dict),
                      detach_cpu_numpy(batch_results))

            try:
                if args.get('net_rend_head', False) and isinstance(rend_dict, dict):
                    extra_keep = {}
                    if ('pupil_c_UV' in rend_dict) and ('eyeball_c_UV' in rend_dict):
                        # make sure we store numpy arrays
                        pc = rend_dict['pupil_c_UV']
                        ec = rend_dict['eyeball_c_UV']
                        if hasattr(pc, 'detach'):
                            pc = pc.detach().cpu().numpy()
                        if hasattr(ec, 'detach'):
                            ec = ec.detach().cpu().numpy()
                        extra_keep['pupil_c_UV']  = pc   # shape: (B*F, 2)
                        extra_keep['eyeball_c_UV'] = ec   # shape: (B*F, 2)
                    if extra_keep:
                        raw = merge_two_dicts(raw, extra_keep)
            except Exception as _e:
                print(f"[WARN] Unable to append UV centers: {_e}")


            try:
                if args.get('net_rend_head', False):
            # rend_dict fields are already numpy earlier (mask/gaze_img clamped & detached)
                    if 'mask' in locals().get('rend_dict', {}):
                        raw['mask_pred_rend'] = rend_dict['mask']             # (B*F,H,W) uint8
                    if 'gaze_img' in locals().get('rend_dict', {}):
                        raw['gaze_img_pred'] = rend_dict['gaze_img']          # (B*F,H,W) uint8
            # pupil/eyeball centers and 3D gaze from the renderer
                    rd_np = detach_cpu_numpy(rend_dict)
                    if 'pupil_c_UV' in rd_np:    raw['pupil_c_UV_pred']  = rd_np['pupil_c_UV']   # (B*F,2)
                    if 'eyeball_c_UV' in rd_np:  raw['eyeball_c_UV_pred']= rd_np['eyeball_c_UV'] # (B*F,2)
                    if 'gaze_vector_3D' in rd_np:raw['gaze_vector_3D']   = rd_np['gaze_vector_3D'] # (B*F,3) or (B*F,4,3)
            except Exception as e:
                print(f"[WARN] Could not add rend_dict fields: {e}")

    # Predicted (from EllSeg head)
            try:
                if args.get('net_ellseg_head', False) and ('mask' in out_dict):
                    raw['mask_pred_ellseg'] = detach_cpu_numpy(out_dict)['mask']   # (B*F,H,W)
            except Exception as e:
                print(f"[WARN] Could not add ellseg mask: {e}")

    # Ground truth (for red GT arrow etc.)
            try:
                gt_np = detach_cpu_numpy(data_dict)
                if 'mask' in gt_np:
                    raw['mask_gt']          = gt_np['mask']               # (B*F,H,W) — will be saved as uint8 below
                if 'gaze_vector' in gt_np:
                    raw['gaze_vector_gt']   = gt_np['gaze_vector']        # (B*F,3)
                if 'eyeball' in gt_np:
                    raw['eyeball_gt']       = gt_np['eyeball']            # (B*F,3) [radius, cx, cy] in px space used in your code
                if 'pupil_center' in gt_np:
                    raw['pupil_center_gt']  = gt_np['pupil_center']       # (B*F,2)
            except Exception as e:
                print(f"[WARN] Could not add GT fields: {e}")
    # ---- END ADD ----




            B = raw[next(iter(raw))].shape[0]   # number of samples
            def ensure_array(x):
                return x if hasattr(x, "shape") else np.full((B,), x, dtype=np.float32)
            test_dict = {k: ensure_array(v) for k,v in raw.items()}
            test_dict_list = convert_to_list_entries(test_dict)

            for idx, entry in enumerate(test_dict_list):
                if idx >= len(data_dict['archName']):
                    # print(f"[WARNING] Skipping idx {idx}, only {len(data_dict['archName'])} entries available.")
                    continue
                ...
                try:
                    img_np = data_dict['image'][idx].detach().cpu().numpy()
                    # convert to uint8 safely
                    if img_np.dtype != np.uint8:
                        maxv = float(np.nanmax(img_np)) if np.isfinite(img_np).any() else 0.0
                        if maxv <= 1.0:
                            img_np = np.clip(img_np * 255.0, 0, 255).round().astype(np.uint8)
                        else:
                            img_np = np.clip(img_np, 0, 255).round().astype(np.uint8)
                    test_results_obj.create_dataset(
                        sample_id + '/image', data=img_np, dtype='uint8', compression='lzf'
                    )
                except Exception as e:
                    print(f"[WARN] Could not save image for {sample_id}: {e}")

                try:
                    img_u8 = data_dict['image_raw_u8'][idx]
                    if torch.is_tensor(img_u8):
                        img_u8 = img_u8.detach().cpu().numpy()
                    test_results_obj.create_dataset(
                        sample_id + '/image_raw_u8', data=img_u8.astype(np.uint8), dtype='uint8', compression='lzf'
                    )
                except Exception as e:
                    print(f"[WARN] Could not save original image for {sample_id}: {e}")

                # Save GT mask if available (only when training-with-mask setup)
                try:
                    if ('mask' in data_dict) and ('mask_available' in data_dict):
                        mask_av = bool(data_dict['mask_available'][idx].item()) \
                                  if torch.is_tensor(data_dict['mask_available'][idx]) \
                                  else bool(data_dict['mask_available'][idx])
                        if mask_av:
                            mask_np = data_dict['mask'][idx].detach().cpu().numpy()
                            mask_np = np.clip(mask_np, 0, 255).astype(np.uint8)
                            test_results_obj.create_dataset(
                                sample_id + '/mask_gt', data=mask_np, dtype='uint8', compression='lzf'
                            )
                except Exception as e:
                    # print(f"[WARN] Could not save mask_gt for {sample_id}: {e}")

                # Save frame_id (helps viz map samples deterministically)
                try:
                    fid = int(data_dict['im_num'][idx].item()) \
                          if torch.is_tensor(data_dict['im_num'][idx]) \
                          else int(data_dict['im_num'][idx])
                    test_results_obj.create_dataset(
                        sample_id + '/frame_id', data=np.array(fid, dtype=np.int32)
                    )
                except Exception as e:
                    print(f"[WARN] Could not save frame_id for {sample_id}: {e}")

        #batch_results['loss'] = loss.item()
        batch_results['loss'] = loss if isinstance(loss, float) else loss.item()
...
def log_wandb(batch_metrics, rend_dict, data_dict, out_dict, loss,
    ...
    if (bt % 100 == 0):
        ...
        wandb.log({'{}/loss'.format(mode): loss if isinstance(loss, float) else loss.item(), 'epoch': epoch, 'batch': bt})
...
def reshape_gt(gt_dict, args):
    train_with_mask = args['loss_w_rend_pred_2_gt_edge'] or args['loss_w_rend_gt_2_pred'] \
                        or args['loss_w_rend_pred_2_gt']
    ##################### Prepare Tensors ############################
    gt_dict['image_raw_u8'] = rearrange(gt_dict['image_raw_u8'], 'b f h w -> (b f) h w')
...
def aggregate_metrics(list_metric_dicts, csv_save_path=None):
    # Aggregate and compute global stats
    keys_list = list_metric_dicts[0].keys() if list_metric_dicts else []
    agg_dict, raw_dict = {}, {}
...
# ✅ Convert all numpy arrays to scalar floats
    for k, v in agg_dict.items():
        if isinstance(v, np.ndarray):
            if v.size == 1:
                agg_dict[k] = float(v)
            else:
                agg_dict[k] = float(np.nanmean(v))
        else:
            agg_dict[k] = float(v)
    ...
    if csv_save_path:
        ...
        for k in raw_dict:
            if 'loss' in k:
                continue
            if 'iou' in k and len(raw_dict[k].shape) > 1:
                for j in range(raw_dict[k].shape[1]):
                    names.append(f'{k}_class{j}')
                    values.append(raw_dict[k][:, j])
            else:
        ...
        if len(values) == 0 or len(names) == 0:
            print("⚠️ Warning: No metrics collected. Skipping CSV creation.")
            return agg_dict
